...
9. For a `comment`, call `claim_agent_comment` before researching so the human sees that work has started. Research additional YouTube sources when asked to verify, find more, or add another perspective. Record new timestamped excerpts, make only the requested canvas change, call `answer_agent_comment`, then wait again. Use `wait_for_agent_comment` only as a one-comment fallback outside a live collaboration session.
