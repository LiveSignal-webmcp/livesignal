...
export default function Home() {
  ...
  const matchingEvents = useMemo(() => {
    ...
    return term ? events.filter((event) => `${event.title} ${event.detail} ${event.quote}`.toLowerCase().includes(term)) : events;
  }, [query]);
  ...
  function addWatchRule(topic = watchTopic) {
    ...
    if (!normalized || watchRules.some((rule) => rule.toLowerCase() === normalized.toLowerCase())) return;
    setWatchRules((rules) => [...rules, normalized]);
  }
  ...
  useEffect(() => {
    ...
    tool("get_stream_info", "Returns the current stream title, creator, status, and available source evidence.", () => ({ title: "The Level Up Live Show", creator: "Maya Chen", status: "live replay", currentEvent: selected.title }));
    tool("get_recent_events", "Returns the latest timestamped livestream events with evidence.", () => events);
    tool("search_stream", "Searches event titles, summaries, and transcript evidence for a topic or phrase.", (input) => {
      ...
      return events.filter((event) => `${event.title} ${event.detail} ${event.quote}`.toLowerCase().includes(search));
  ...
  return <main>
    ...
    <section className="workspace" id="watch" aria-label="Live stream monitoring demo">
      ...
      <div className="monitor-grid">
        ...
        <aside className="events-panel" id="events"><div className="panel-heading"><div><p className="overline">LIVE TIMELINE</p><h3>Events that matter</h3></div><span>{events.length} found</span></div><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search this stream" aria-label="Search this stream" /></label><div className="event-list">{matchingEvents.map((event) => <button type="button" className={`event-row ${event.id === selectedId ? "selected" : ""}`} onClick={() => selectEvent(event)} key={event.id}><time>{event.time}</time><span className={`event-dot ${event.type.toLowerCase()}`} /><span><b>{event.title}</b><small>{event.detail}</small></span></button>)}{!matchingEvents.length && <p className="empty-state">No matching events yet.</p>}</div></aside>
      </div>
